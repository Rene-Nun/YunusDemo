...
const IPhone  = ({size=20,color="currentColor"}) => <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M22 16.92v3a2 2 0 01-2.18 2 19.79 19.79 0 01-8.63-3.07A19.5 19.5 0 013.07 9.8a19.79 19.79 0 01-3.07-8.67A2 2 0 012 0h3a2 2 0 012 1.72c.127.96.361 1.903.7 2.81a2 2 0 01-.45 2.11L6.09 7.91a16 16 0 006 6l1.27-1.27a2 2 0 012.11-.45c.907.339 1.85.573 2.81.7A2 2 0 0122 16.92z"/></svg>;
const ILock   = ({size=16,color="currentColor"}) => <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="11" width="18" height="11" rx="2" ry="2"/><path d="M7 11V7a5 5 0 0110 0v4"/></svg>;
...
const ANALYSIS_STEPS = [
  {text:"Conectando con tu banco de forma segura...", sub:"Open Banking"},
  {text:"Analizando metadatos financieros...",        sub:"Credolab"},
  {text:"Calculando tu capacidad de pago...",         sub:"Factor b de Yunus"},
  {text:"¡Perfil aprobado!",                          sub:"", done:true},
];
...
function LoginScreen({onContinue}) {
  ...
  return (
    <div style={{flex:1,display:"flex",flexDirection:"column",background:"#fff"}}>
      ...
      {/* Hero area */}
      <div style={{flex:1,display:"flex",flexDirection:"column",justifyContent:"center",padding:"0 32px 24px"}}>

        {/* Logo */}
        <div style={{marginBottom:48}}>
          <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:6}}>
            <div style={{width:40,height:40,borderRadius:12,background:`linear-gradient(135deg,${B},${BD})`,display:"flex",alignItems:"center",justifyContent:"center"}}>
              <span style={{color:"#fff",fontWeight:800,fontSize:18}}>Y</span>
            </div>
            <span style={{fontSize:22,fontWeight:800,color:"#0F172A",letterSpacing:"-0.5px"}}>yunus</span>
          </div>
          <div style={{width:32,height:3,borderRadius:2,background:B}}/>
        </div>

        {/* Headline */}
        <div style={{marginBottom:40}}>
          <h1 style={{fontSize:30,fontWeight:800,color:"#0F172A",lineHeight:1.15,margin:"0 0 12px",letterSpacing:"-0.5px"}}>
            Asegura tu lugar<br/>en el Mundial<br/>
            <span style={{color:B}}>2026.</span>
          </h1>
          <p style={{fontSize:15,color:"#64748B",margin:0,lineHeight:1.5}}>
            Descubre tus boletos pre-aprobados sin afectar tu buró.
          </p>
        </div>

        {/* Phone input */}
        <div style={{marginBottom:16}}>
          <p style={{fontSize:12,fontWeight:600,color:"#64748B",margin:"0 0 8px",letterSpacing:"0.04em",textTransform:"uppercase"}}>Número de teléfono</p>
          <div style={{display:"flex",alignItems:"center",gap:12,background:"#F8FAFC",border:`2px solid ${isValid ? B : "#E2E8F0"}`,borderRadius:16,padding:"14px 16px",transition:"border-color 0.2s"}}>
            <div style={{display:"flex",alignItems:"center",gap:6,borderRight:"1px solid #E2E8F0",paddingRight:12,flexShrink:0}}>
              <span style={{fontSize:16}}>🇲🇽</span>
              <span style={{fontSize:14,fontWeight:600,color:"#64748B"}}>+52</span>
            </div>
            <input
              type="tel"
              inputMode="numeric"
              ...
              style={{flex:1,background:"none",border:"none",outline:"none",fontSize:17,fontWeight:600,color:"#0F172A",letterSpacing:"0.5px"}}
            />
            {isValid && <ICheck size={18} color={B}/>}
          </div>
        </div>
        ...
        <button
          ...
          style={{width:"100%",padding:"16px 0",borderRadius:16,border:"none",fontSize:16,fontWeight:700,cursor:isValid?"pointer":"default",transition:"all 0.2s",
            background:isValid?`linear-gradient(135deg,${B},${BD})`:"#E2E8F0",
            color:isValid?"#fff":"#94A3B8",
            boxShadow:isValid?"0 8px 24px rgba(65,112,140,0.35)":"none",
        ...
        <div style={{display:"flex",alignItems:"center",justifyContent:"center",gap:6,marginTop:16}}>
          <ILock size={13} color="#94A3B8"/>
          <span style={{fontSize:12,color:"#94A3B8"}}>Sin consulta al buró · 100% seguro</span>
        </div>
      </div>

      {/* Bottom decoration */}
      <div style={{padding:"0 32px 32px"}}>
        <div style={{borderTop:"1px solid #F1F5F9",paddingTop:20,display:"flex",justifyContent:"center",gap:24}}>
          {["FIFA Licenciado","Open Banking","256-bit SSL"].map(t=>(
            <div key={t} style={{textAlign:"center"}}>
              <p style={{fontSize:10,color:"#CBD5E1",fontWeight:600,margin:0,letterSpacing:"0.04em"}}>{t}</p>
            </div>
          ))}
        </div>
      </div>
...
function AnalysisScreen({onDone}) {
  ...
  useEffect(()=>{
    ...
    // Step cycling: 4 steps over ~4.2s
    const timings = [0, 1000, 2200, 3300];
    const timers = timings.map((t,i)=>
      setTimeout(()=>{
        setFadeIn(false);
        setTimeout(()=>{ setStep(i); setFadeIn(true); }, 200);
      }, t)
    );

    // Transition out after 4.5s
    const doneTimer = setTimeout(()=> onDone(), 4700);

    return ()=>{ clearInterval(pTimer); timers.forEach(clearTimeout); clearTimeout(doneTimer); };
  },[]);

  const isDone = step === 3;

  return (
    <div style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",background:`linear-gradient(160deg,${BD} 0%,${B} 50%,"#5a8fa8" 100%)`,padding:"40px 32px",position:"relative",overflow:"hidden"}}>
      ...
      <div style={{marginBottom:48,textAlign:"center"}}>
        <div style={{width:56,height:56,borderRadius:18,background:"rgba(255,255,255,0.15)",display:"flex",alignItems:"center",justifyContent:"center",margin:"0 auto 10px",backdropFilter:"blur(10px)"}}>
          <span style={{color:"#fff",fontWeight:800,fontSize:24}}>Y</span>
        </div>
        <p style={{color:"rgba(255,255,255,0.7)",fontSize:13,margin:0,letterSpacing:"0.08em",textTransform:"uppercase",fontWeight:600}}>yunus</p>
      </div>
      ...
      <div style={{position:"relative",width:120,height:120,marginBottom:40}}>
        ...
        <div style={{position:"absolute",inset:0,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center"}}>
          {isDone
            ...
            : <span style={{fontSize:28,fontWeight:800,color:"#fff"}}>{Math.round(progress)}%</span>
          }
        </div>
      </div>

      {/* Step text */}
      <div style={{textAlign:"center",minHeight:64,opacity:fadeIn?1:0,transition:"opacity 0.2s"}}>
        <p style={{fontSize:17,fontWeight:700,color:"#fff",margin:"0 0 6px",lineHeight:1.3}}>
          {ANALYSIS_STEPS[step].text}
        </p>
        {ANALYSIS_STEPS[step].sub && (
          <p style={{fontSize:12,color:"rgba(255,255,255,0.55)",margin:0,fontWeight:500,letterSpacing:"0.04em"}}>
            {ANALYSIS_STEPS[step].sub}
          </p>
        )}
      </div>
      ...
      <div style={{display:"flex",gap:6,marginTop:32}}>
        {ANALYSIS_STEPS.map((_,i)=>(
          <div key={i} style={{width:i===step?20:6,height:6,borderRadius:3,background:i<=step?"#fff":"rgba(255,255,255,0.25)",transition:"all 0.3s"}}/>
        ))}
      </div>
      ...
      <div style={{marginTop:36,width:"100%",maxWidth:280}}>
        {ANALYSIS_STEPS.map((s,i)=>(
          <div key={i} style={{display:"flex",alignItems:"center",gap:10,marginBottom:i<3?10:0,opacity:i<=step?1:0.3,transition:"opacity 0.4s"}}>
            <div style={{width:20,height:20,borderRadius:"50%",flexShrink:0,display:"flex",alignItems:"center",justifyContent:"center",
              background:i<step?"rgba(255,255,255,0.9)":i===step?"rgba(255,255,255,0.2)":"rgba(255,255,255,0.1)"}}>
              {i<step
                ? <ICheck size={11} color={B}/>
                : <span style={{width:6,height:6,borderRadius:"50%",background:"rgba(255,255,255,0.6)",display:"block"}}/>
              }
            </div>
            <span style={{fontSize:12,color:i<=step?"rgba(255,255,255,0.9)":"rgba(255,255,255,0.4)",fontWeight:i===step?600:400,flex:1}}>{s.text}</span>
          </div>
        ))}
      </div>
    </div>
  ...
  return (
    <div style={{marginBottom:4}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}>
        ...
        <div style={{display:"flex",gap:4}}>
          <button onClick={()=>setCi(i=>(i-1+PRE.length)%PRE.length)} style={{width:20,height:20,borderRadius:"50%",border:"none",background:"#F1F5F9",cursor:"pointer",fontSize:13,color:"#64748B",display:"flex",alignItems:"center",justifyContent:"center"}}>‹</button>
          <button onClick={()=>setCi(i=>(i+1)%PRE.length)}            style={{width:20,height:20,borderRadius:"50%",border:"none",background:"#F1F5F9",cursor:"pointer",fontSize:13,color:"#64748B",display:"flex",alignItems:"center",justifyContent:"center"}}>›</button>
        </div>
      </div>
  ...
  return (
    <div style={{display:"flex",justifyContent:u?"flex-end":"flex-start",marginBottom:14}}>
      ...
      <div style={{maxWidth:"84%"}}>
        ...
        {msg.type==="carousel"  && <div style={{width:290}}><CarouselCard ci={msg.ci} setCi={msg.setCi}/></div>}
          ...
          <div>
            ...
            {msg.type==="soft_no" && (
              <div style={{background:"#FFFBEB",border:"1px solid #FDE68A",borderRadius:12,padding:"9px 12px",marginBottom:8}}>
                <p style={{fontSize:11,fontWeight:700,color:"#B45309",margin:"0 0 3px",display:"flex",alignItems:"center",gap:5}}><IWarn size={12}/> Análisis de Riesgo</p>
    ...
    <div style={{flex:1,overflowY:"auto",display:"flex",flexDirection:"column"}}>
      ...
      {all.length===0 ? (
        <div style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"40px 32px",background:"#F8FAFC"}}>
          <div style={{width:72,height:72,borderRadius:24,background:BL,display:"flex",alignItems:"center",justifyContent:"center",marginBottom:16,fontSize:32}}>🎫</div>
          <p style={{fontSize:17,fontWeight:700,color:"#0F172A",margin:"0 0 8px",textAlign:"center"}}>Tu bóveda está vacía</p>
          <p style={{fontSize:13,color:"#94A3B8",margin:"0 0 24px",textAlign:"center",lineHeight:1.5}}>Cuando adquieras un boleto aparecerá aquí con su plan de pagos.</p>
          <div style={{background:"#fff",border:"1px dashed #CBD5E1",borderRadius:16,padding:"14px 18px",width:"100%",maxWidth:280}}>
            ...
            {["Descubre boletos en el chat con Yunus","Yunus analiza tu liquidez y propone un plan","Paga el enganche y el boleto aparece aquí"].map((s,i)=>(
        ...
        <div style={{padding:"4px 20px 24px",background:"#F8FAFC",flex:1}}>
          <div style={{background:BL,borderRadius:14,padding:"11px 14px",display:"flex",justifyContent:"space-between",marginBottom:14}}>
            ...
            <div style={{textAlign:"right"}}><p style={{fontSize:11,color:"#64748B",margin:0}}>Total pagado</p><p style={{fontSize:16,fontWeight:800,color:"#0F172A",margin:0}}>${all.reduce((a,t)=>a+parseInt(t.pagado.replace(/[$,]/g,"")),0).toLocaleString("es-MX")}</p></div>
          </div>
          {all.map(t=>(
        ...
        <div onClick={()=>setResell(null)} style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.45)",zIndex:50,display:"flex",alignItems:"flex-end",justifyContent:"center"}}>
          <div onClick={e=>e.stopPropagation()} style={{background:"#fff",borderRadius:"24px 24px 0 0",padding:"20px 20px 32px",width:"100%",maxWidth:390}}>
            ...
            <div style={{background:"#FFF7ED",borderRadius:14,padding:"12px 14px",marginBottom:14}}>
              <p style={{fontSize:11,fontWeight:700,color:"#B45309",margin:"0 0 4px",display:"flex",alignItems:"center",gap:5}}><IWarn/> Precio sugerido</p>
...
function Discovery({paidTickets,onPay}) {
  ...
  const [msgs,setMsgs]    = useState([
    {id:0,role:"ai",type:"text",    content:"¡Hola! 👋 Soy Yunus. Analicé tu perfil y tengo estas opciones pre-aprobadas para ti:",time:"10:00 AM"},
    {id:1,role:"ai",type:"carousel",ci:0,time:"10:00 AM"},
    {id:2,role:"ai",type:"text",    content:"¿Te interesa alguno? O cuéntame a dónde quieres ir al Mundial 2026 🏆",time:"10:00 AM"},
  ]);
  const bottomRef = useRef(null);
  useEffect(()=>{ bottomRef.current?.scrollIntoView({behavior:"smooth"}); },[msgs]);

  const updateCi = (newCi) => {
    setCi(newCi);
    setMsgs(p=>p.map(m=>m.type==="carousel"?{...m,ci:newCi,setCi:updateCi}:m));
  };
  useEffect(()=>{
    setMsgs(p=>p.map(m=>m.type==="carousel"?{...m,setCi:updateCi}:m));
  },[]);
  ...
  const send=()=>{
    ...
    if(lo.includes("mundial")||lo.includes("cdmx")||lo.includes("azteca")){
      runThink(TH_M,()=>setMsgs(p=>[...p,
        {id:Date.now(),role:"ai",type:"text",content:"¡Encontré algo perfecto! 🏆 Aquí está tu plan:",time:nowT()},
      ...
      runThink(TH_V,()=>setMsgs(p=>[...p,
        {id:Date.now(),role:"ai",type:"soft_no",
         content:"Financiar el VIP comprometería tus finanzas este mes. 💡 Mi contraoferta saludable: 2 boletos Cat. 3 en MTY.",
         warning:"El VIP representa un sobreendeudamiento del 340% vs tu capacidad actual.",
      ...
      setTimeout(()=>setMsgs(p=>[...p,{id:Date.now(),role:"ai",type:"text",content:"Prueba: \"Quiero ir al mundial en CDMX\" o \"Quiero boletos VIP\" 😊",time:nowT()}]),600);
    }
  };
...
function Marketplace() {
  ...
  const shown=filter==="Todos"?MKT:filter==="MTY"?MKT.filter(t=>t.city==="Monterrey"):filter==="CDMX"?MKT.filter(t=>t.city==="CDMX"):MKT.filter(t=>["Dallas","Los Ángeles"].includes(t.city));
    ...
    <div style={{flex:1,overflowY:"auto",display:"flex",flexDirection:"column"}}>
      <div style={{padding:"16px 20px 12px",background:"#fff",flexShrink:0}}>
        ...
        <div style={{display:"flex",gap:6,overflowX:"auto",paddingBottom:2}}>
          {filters.map(f=>(
            <button key={f} onClick={()=>setFilter(f)} style={{padding:"6px 14px",borderRadius:20,border:"none",fontSize:12,fontWeight:600,cursor:"pointer",whiteSpace:"nowrap",background:filter===f?B:"#F1F5F9",color:filter===f?"#fff":"#64748B",flexShrink:0,transition:"all 0.2s"}}>{f}</button>
          ))}
        </div>
      </div>
        ...
        {shown.map(t=>(
          <div key={t.id} style={{background:"#fff",borderRadius:18,border:"1px solid #F1F5F9",boxShadow:"0 1px 4px rgba(0,0,0,0.05)",marginBottom:12,padding:"14px"}}>
            ...
            <div style={{borderTop:"1px solid #F8FAFC",paddingTop:10,display:"flex",alignItems:"center",justifyContent:"space-between"}}>
              ...
              <button style={{padding:"8px 16px",borderRadius:12,background:B,color:"#fff",border:"none",fontSize:12,fontWeight:600,cursor:"pointer",flexShrink:0}}>Comprar con Plan</button>
            </div>
          </div>
        ))}
      </div>
...
// ══════════════════════════════════════════════════════════════════
// MAIN APP — screen state: "login" | "analysis" | "app"
// ══════════════════════════════════════════════════════════════════
export default function App() {
  ...
  if(screen==="login") {
    return (
      <div style={{minHeight:"100vh",background:"#CBD5E1",display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"-apple-system,BlinkMacSystemFont,'SF Pro Display',Helvetica,sans-serif"}}>
        <div style={{width:390,height:844,background:"#fff",borderRadius:44,overflow:"hidden",boxShadow:"0 30px 80px rgba(0,0,0,0.28)",display:"flex",flexDirection:"column"}}>
          ...
          <LoginScreen onContinue={()=>setScreen("analysis")}/>
        </div>
      </div>
    );
  }
  ...
  if(screen==="analysis") {
    return (
      <div style={{minHeight:"100vh",background:"#CBD5E1",display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"-apple-system,BlinkMacSystemFont,'SF Pro Display',Helvetica,sans-serif"}}>
        <div style={{width:390,height:844,borderRadius:44,overflow:"hidden",boxShadow:"0 30px 80px rgba(0,0,0,0.28)",display:"flex",flexDirection:"column"}}>
          <div style={{height:44,background:BD,display:"flex",alignItems:"center",justifyContent:"space-between",padding:"0 24px",flexShrink:0}}>
            <span style={{fontSize:14,fontWeight:700,color:"rgba(255,255,255,0.8)"}}>9:41</span>
            <span style={{fontSize:12,color:"rgba(255,255,255,0.5)",letterSpacing:2}}>●●●●</span>
          </div>
          <AnalysisScreen onDone={()=>setScreen("app")}/>
        </div>
      </div>
    );
  }
        ...
        <div style={{background:"#fff",padding:"8px 16px 10px",borderBottom:"1px solid #F1F5F9",flexShrink:0}}>
          <div style={{display:"flex",background:"#F1F5F9",borderRadius:16,padding:3,gap:2}}>
            {tabs.map(({label,Icon,badge},i)=>(
              <button key={i} onClick={()=>setTab(i)} style={{flex:1,display:"flex",alignItems:"center",justifyContent:"center",gap:5,padding:"7px 4px",borderRadius:13,border:"none",cursor:"pointer",fontSize:11,fontWeight:600,transition:"all 0.2s",position:"relative",background:tab===i?"#fff":"transparent",color:tab===i?B:"#94A3B8",boxShadow:tab===i?"0 1px 8px rgba(0,0,0,0.10)":"none"}}>
                <Icon size={16} color={tab===i?B:"#94A3B8"}/>
                {label}
                {badge>0&&<span style={{position:"absolute",top:2,right:3,width:16,height:16,borderRadius:"50%",background:"#E11D48",color:"#fff",fontSize:9,fontWeight:800,display:"flex",alignItems:"center",justifyContent:"center"}}>{badge}</span>}
              </button>
            ))}
          </div>
          <div style={{display:"flex",justifyContent:"center",gap:5,marginTop:7}}>
            {[0,1,2].map(i=><div key={i} style={{width:i===tab?20:5,height:5,borderRadius:3,background:i===tab?B:"#E2E8F0",transition:"all 0.3s"}}/>)}
          </div>
        </div>

        <div style={{flex:1,overflow:"hidden",display:"flex",flexDirection:"column"}}>
          <div style={{display:tab===0?"flex":"none",flex:1,overflow:"hidden",flexDirection:"column"}}><Boveda tickets={vault}/></div>
          <div style={{display:tab===1?"flex":"none",flex:1,overflow:"hidden",flexDirection:"column"}}><Discovery paidTickets={vault} onPay={addTicket}/></div>
          <div style={{display:tab===2?"flex":"none",flex:1,overflow:"hidden",flexDirection:"column"}}><Marketplace/></div>
        </div>
      </div>
    </div>
  );
